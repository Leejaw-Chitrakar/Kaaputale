    ...
    return (
        <div className="ad-overlay" onClick={onClose}>
            <div className="ad-modal ad-modal--form" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="ad-form-section">
                        ...
                        <div className="ad-form-group">
                            ...
                            <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                                Upload to Firebase Storage or any image host, then paste the URL here.
                            </span>
                        </div>
                ...
                <div className="ad-modal__footer">
                    <button className="ad-btn ad-btn--ghost" onClick={onClose} disabled={loading}>Cancel</button>
                    <button className="ad-btn ad-btn--primary" onClick={handleSubmit} disabled={loading}>
                        {loading ? <Spinner size={16} /> : isEdit ? "Save Changes" : "Add Product"}
                    </button>
                </div>
            </div>
        </div>
...
export default function AdminDashboard({ onExit }) {
    ...
    return (
        <div className="ad-root" data-theme={theme}>
            ...
            {/* ── Modals ───────────────────────────────────────── */}
            {addModal && (
                <ProductFormModal onSave={handleAdd} onClose={() => setAddModal(false)} loading={mutLoading} />
            )}
            {editTarget && (
                <ProductFormModal initial={editTarget} onSave={handleEdit}
                    onClose={() => setEditTarget(null)} loading={mutLoading} />
            )}
            {deleteTarget && (
